module.exports = {
  ...
  fn: async function ({
    ...
    var dontActuallySend = sails.config.environment === 'test' || isToAddressConsideredFake;
    if (dontActuallySend) {
      sails.log(
          ...
          if (err) {
            sails.log.error(
              'Background instruction failed:  Could not deliver email:\n' +
                util.inspect(
                  { template, templateData, to, toName, subject, from, fromName, layout, ensureAck, bcc, attachments },
                  { depth: null },
                ) +
                '\n',
              'Error details:\n' + util.inspect(err),
